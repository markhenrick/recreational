...
""";
	private static final String MY_INPUT = TestUtil.getResourceAsString("AoC/input/2020/day07.txt");
	...
	static Stream<Arguments> parse() {
		return Stream.of(
			arguments("dark orange bags contain 3 bright white bags, 4 muted yellow bags.",
				new Day07.Production("dark orange", Map.of("bright white", 3, "muted yellow", 4))),
			arguments("bright white bags contain 1 shiny gold bag.",
				new Day07.Production("bright white", Map.of("shiny gold", 1))),
			arguments("faded blue bags contain no other bags.",
				new Day07.Production("faded blue", Collections.emptyMap()))
		);
	}

	@ParameterizedTest
	@MethodSource
	void parse(final String input, final Day07.Production expected) {
		AssertionsForClassTypes.assertThat(Day07.Production.parse(input)).isEqualTo(expected);
	}
}
